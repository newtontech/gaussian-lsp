...
def _load_intent(path: Path) -> dict[str, Any] | None:
    ...
    (e.g. ``software_version``, ``mem_warning_mb``). It is a workspace-local
    artifact, never a MatMaster/Bohrium runtime concept.
    """
    case_dir = path if path.is_dir() else path.parent
    intent_path = case_dir / ".gaussian-lsp" / "intent.json"
...
def _looks_like_workspace(path: Path) -> bool:
    ...
    the legacy single-file lint path so callers that progressively build
    inputs are not flooded with blocking missing-route errors before the input
    exists.
    """
...
def _resolve_input_path(path: Path) -> Path:
    ...
    if path.is_dir():
        # Prefer an explicit .gjf/.com, otherwise the first file with a route.
        candidates = sorted(p for p in path.iterdir() if p.is_file())
        for candidate in candidates:
            if candidate.suffix.lower() in {".gjf", ".com"}:
                return candidate
        from .preflight import looks_like_gaussian_workspace

        for candidate in candidates:
            if looks_like_gaussian_workspace(candidate):
                return candidate
...
def check_path(path: Path) -> dict[str, Any]:
    ...
    # only for a real generated-input workspace (a directory or .gjf). A bare
    # single non-Gaussian file path keeps the legacy single-file behavior so
...
def manifest_path(path: Path | None = None) -> dict[str, Any]:
    ...
    ``.gaussian-lsp/fixtures.json`` are merged in so the parent probe can
    confirm a case directory exercises the documented codes.
    """
    ...
    if path is not None:
        case_dir = path if path.is_dir() else path.parent
        fixtures_path = case_dir / ".gaussian-lsp" / "fixtures.json"
