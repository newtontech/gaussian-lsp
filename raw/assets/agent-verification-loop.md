---
source: docs/agent-verification-loop.md
extracted: 2026-06-12
---

# Agent Verification Loop for Gaussian LSP

## Overview

This document describes the recommended verification loop for coding agents
(Claude Code, OpenCode, Codex) working with Gaussian input files using the
Gaussian LSP server.
...
The Gaussian LSP provides custom commands for agent consumption:

- `Gaussian.diagnosticsSnapshot` — Returns JSON snapshot of all diagnostics.
...
```python
config = TestRunnerConfig(executable="Gaussian", enabled=True)
