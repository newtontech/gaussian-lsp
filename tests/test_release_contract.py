"""Release contract for the Gaussian-LSP 0.2.12 publication path."""
...
def test_release_version_is_consistent_across_all_manifests() -> None:
    """Every runtime and integration manifest must advertise the release version."""
    pyproject = (REPO_ROOT / "pyproject.toml").read_text(encoding="utf-8")
    pyproject_match = re.search(r'^version = "([^"]+)"$', pyproject, re.MULTILINE)
    init_text = (REPO_ROOT / "src/gaussian_lsp/__init__.py").read_text(encoding="utf-8")
    ...
    agent_api = (REPO_ROOT / "src/parsers/agent-api.ts").read_text(encoding="utf-8")
    ...
    assert f"version: '{RELEASE_VERSION}'" in agent_api
