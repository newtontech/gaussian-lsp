#!/usr/bin/env python3
"""Install a Gaussian-LSP wheel in isolation and smoke its public CLIs."""
...
def smoke(wheel: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="gaussian-lsp-wheel-") as temporary:
        ...
        server = bindir / f"gaussian-lsp{executable_suffix}"
        tool = bindir / f"gaussian-lsp-tool{executable_suffix}"
        ...
        installed_version = _run(
            [
                str(python),
                "-c",
                "from importlib.metadata import version; print(version('gaussian-lsp'))",
            ],
            cwd=isolated,
        ).stdout.strip()
        ...
        valid = ROOT / "tests/fixtures/valid/water_sp.gjf"
        ...
        invalid = ROOT / "tests/fixtures/invalid/missing_charge_mult.gjf"
        ...
        log = ROOT / "tests/fixtures/log/scf_not_converged.out"
        log_payload = _json(
            _run(
